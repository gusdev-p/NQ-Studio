...
export const nq = {
    ...
    /**
     * ## joinPath
     * Receives all the paths and join then into a single path.
     ...
     * @param path The path to receive the directory name.
     * @returns The path directory name.
     */
    ...
    /**
     * ## resolvePath
     * Receives all the paths and resolve them
     * "~/something" -> "/home/your-user/something".
     * @param paths The paths to resolve.
     * @returns The paths resolved.
     */
    ...
    /**
     * ## initNqDir
     * Init the `globalRoot` with the `.nq` directory and the
     * `settings.json` file.
     * @returns Nothing.
     */
    ...
    /**
     * ## getLocalProperty
     * Search for the key inside the `settings.json`(generated by the **initNqDir**)
     * and returns the key value.
     * @param key The key to search the value.
     * @returns The key value.
     */
    getLocalProperty(key: string): Promise<any> {
        return ipcRenderer.invoke("getLocalProperty", key);
    },

    /**
     * ## setLocalProperty
     * Receives a key and a value, set to the `settings.json`
     * and saves the `settings.json`.
    ...
    /**
     * ## openDevTools
     * Opens the dev tools of the main window.
     * Normally used for debug.
     * @param modeToSet - The mode of the devTools window
     * @returns Nothing.
     */
    openDevTools(modeToSet: ('left' | 'right' | 'bottom' | 'undocked' | 'detach') = "right"): Promise<void> {
        return ipcRenderer.invoke("openDevTools", modeToSet)
    },
...
export const server = {
    /**
     * ## openHttpServer
     * Hosts a server using the specified `port`.
     * Normally used for HTML previews.
     * @param root - The directory to be hosted.
     * @param port - The server port. The default is automatic port.
     * @returns - The server URL.
     */
    openHttpServer(root: string, port: number = 0): Promise<string> {
        return ipcRenderer.invoke("server:openHttpServer", root, port);
    },
    ...
    closeHttpServer(): Promise<void> {
        return ipcRenderer.invoke("server:closeHttpServer");
    },
    ...
    getServerUrl(): Promise<string> {
        return ipcRenderer.invoke("server:getHttpUrl");
    }
};

export const make = {
    parse(filePath: string): Promise<any> {
        return ipcRenderer.invoke("make:parse", filePath)
    }
}
